...
def build_and_save(docs, size=2000, path=VOCAB_FILE):
    freq = {}
    for d in docs:
        for t in set(tokenize(d)): freq[t] = freq.get(t,0)+1
    ...
    with open(path,'wb') as f: pickle.dump({'vocab':vocab,'idf':idf},f)
    return vocab, idf

def load_state(path=VOCAB_FILE):
    if os.path.exists(path):
        with open(path,'rb') as f: return pickle.load(f)
    return None

def embed_with(text, vocab, idf):
...
def embed(text):
    ...
    return embed_with(text, s['vocab'], s['idf'])
